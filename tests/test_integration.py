...
from retrogit import (
    get_config_path,
    load_config,
    get_last_commit_date,
    calculate_next_commit_date,
    setup_git_dates
)
...
def test_initial_commit_date(temp_git_repo, temp_config):
    ...
    frozen_datetime = datetime.strptime(frozen_time, '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
    ...
    subprocess.run(['git', 'commit', '-m', 'Initial commit'], env=env, check=True)
    
    # Verify the commit date
    commit = next(repo.iter_commits())
    commit_date = datetime.fromtimestamp(commit.committed_date, tz=timezone.utc)
    expected_datetime = datetime.strptime(expected_date, '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
...
def test_subsequent_commit_dates(temp_git_repo, temp_config):
    ...
    with freeze_time('2024-01-01 12:00:00'):
        ...
        env['GIT_AUTHOR_DATE'] = date_str
        env['GIT_COMMITTER_DATE'] = date_str
        subprocess.run(['git', 'commit', '-m', 'Second commit'], env=env, check=True)
...
def test_pre_commit_hook(temp_git_repo, temp_config):
    """Test that the pre-commit hook sets dates correctly."""
    # Install the pre-commit hook
    retrogit_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'retrogit', '__init__.py'))
    subprocess.run(['python3', retrogit_path, 'install'], cwd=temp_git_repo, check=True)
